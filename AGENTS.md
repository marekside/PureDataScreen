...
- **Stamina field** (StaminaField): two-tank model — W'bal (fast anaerobic capacity, ~20 min to empty at FTP) + Energy (slow glycogen-like endurance, ~5 h at zone 2, ~90 min at FTP). Both driven by the same intensity function below AeT/top-of-Z2 → negative recovery, linear ramp through Z3/Z4/Z5 to full drain at top-of-Z5. Displayed value = `min(W'bal, Energy)` so the athlete always sees the current bottleneck. Thresholds from configured power zones (preferred, `maxZ2`/`maxZ5`) or HR zones, with FTP-based fallback (`0.75×FTP` / `1.20×FTP`). Color coding: >50 green, 20-50 yellow, <20 red. Trend arrows use ASCII `+`/`-` (`TREND_THRESHOLD = 0.05`).
